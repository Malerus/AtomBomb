...
/obj/machinery/trading_machine/proc/add_item(obj/item/Itm, mob/living/carbon/human/user)
	...
	if(is_available_category(Itm) && is_acceptable_item_state(Itm))
		...
		to_chat(usr, "You loaded [Itm.name] to vending machine. New price - [content[Itm]] USD..")
		src.ui_interact(usr)
	else
...
/obj/machinery/trading_machine/proc/get_ui_content(state)
	...
	switch(state)
		...
		if(STATE_IDLE)
			...
			if(content.len == 0)
				...
				for(var/obj/item/Itm in content)
					...
					dat += "<a href='byond://?src=\ref[src];vend=[item_name];current_price=[price]'>[Itm.name] | [price] USD</a> "
		...
		if(STATE_SERVICE)
			...
			dat += "<div class='statusDisplay'>"
			dat += "<font color='green'>USD stored - [stored_caps]</font>"
			...
			if(content.len == 0)
				...
				for(var/obj/item/Itm in content)
					var/item_name = url_encode(Itm.name)
					var/price = content[Itm]
					dat += "<b>[Itm.name]</b> - [content[Itm]] USD"
		...
		if(STATE_VEND)
			dat += "<h3>Select an item</h3>"
			dat += "<div class='statusDisplay'>"
			dat += "<font color = 'red'>Waiting for [expected_price] USD!</font>"
